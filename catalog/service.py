...
from .models import Brand  # import models that in present in models.py
...
def get_products(filters):  # get function to get products with filters
    ...
    if filters.get("brand"):  # filter by brand
        queryset = queryset.filter(
            brand=filters["brand"]
        )  # select specific brand if call
    if filters.get("category"):  # filter by category
        queryset = queryset.filter(
            category=filters["category"]
        )  # select specific category if call
    if filters.get("in_stock") is not None:  # filter by stock availability
        queryset = queryset.filter(
            in_stock=filters["in_stock"].lower() in ["true", "1"]
        )  # select specific stock availability if call and then convert to boolean
    if filters.get(
        "min_price"
    ):  # filter by minimum price (gte means greater than or equal to)
        queryset = queryset.filter(
            price__gte=filters["min_price"]
        )  # select specific minimum price if call
    if filters.get(
        "max_price"
    ):  # filter by maximum price (lte means less than or equal to)
        queryset = queryset.filter(
            price__lte=filters["max_price"]
        )  # select specific maximum price if call
    ...
    if filters.get("ordering"):  # order by field
        queryset = queryset.order_by(
            filters["ordering"]
        )  # order by specific field if call
...
def get_cart_items(request):  # get cart items based on user authentication
    if request.user.is_authenticated:  # if user is authenticated
        ...
        ):  # for anonymous users, create a session if not exists
...
def add_to_cart(
    ...
    if request.user.is_authenticated:  # if user is authenticated
        cart_item, created = CartItem.objects.get_or_create(  # get or create cart item
            ...
            defaults={
                "quantity": quantity
            },  # if not exists, create with default quantity
        )
    else:
        if (
            not request.session.session_key
...
):  # update cart item quantity based on user authentication
